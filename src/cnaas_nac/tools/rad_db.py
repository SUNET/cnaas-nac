...
def get_connstrs(source: str, target: str, username: str, password: str) -> tuple:
...
def edit_replies(diff, connstr, remove=False):
    ...
    with sqla_session(connstr) as session:
        for diff_user in diff:
            ...
            reply = (
                session.query(Reply)
                .filter(Reply.username == username)
                .filter(Reply.attribute == attribute)
                .one_or_none()
            )
        ...
        for diff_user in nas_diff:
            ...
            if remove and nas is not None:
                ...
                if (
                    ...
                    or nas_port_id != nas.nas_port_id
                    or nas_ip_address != nas.nas_ip_address
                    or calling_station_id != nas.calling_station_id
                    or called_station_id != nas.called_station_id
                ):
...
def copy_accounting(diffs, connstr, table=Accounting):
    ...
    with sqla_session(connstr) as session:
        for diff in diffs:
            ...
            acct = (
                session.query(table)
                .filter(Accounting.acctuniqueid == acctuniqueid)
                .one_or_none()
            )
...
def edit_userinfo(userinfo_diff, connstr, table=UserInfo, remove=False):
    ...
    with sqla_session(connstr) as session:
        for diff_user in userinfo_diff:
            ...
            if "access_start" in diff_user:
                access_start = diff_user["access_start"]

            if "access_stop" in diff_user:
                access_stop = diff_user["access_stop"]
